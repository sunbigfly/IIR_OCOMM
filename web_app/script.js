...
function initializeDropdowns() {
    // 获取唯一的ROUTE值
    const uniqueRoutes = [...new Set(allData.map(item => item.ROUTE).filter(Boolean))].sort();
    populateDropdown(elements.routeSearch, uniqueRoutes);
    
    // 获取唯一的DOSAGE_FORM值
    const uniqueDosageForms = [...new Set(allData.map(item => item.DOSAGE_FORM).filter(Boolean))].sort();
    populateDropdown(elements.dosageFormSearch, uniqueDosageForms);
}
...
function bindEvents() {
    ...
    // 回车键搜索
    [elements.ingredientSearch, elements.casSearch, elements.uniiSearch].forEach(input => {
...
function performSearch() {
    ...
    filteredData = allData.filter(item => {
        ...
        if (filters.ingredient) {
            ...
            const ingredientNameCn = (item['INGREDIENT_NAME(中文名)'] || '').toLowerCase();
            if (!ingredientName.includes(filters.ingredient) && 
...
function createTableRow(item) {
    ...
    // 其他字段
    const fields = ['CAS_NUMBER', 'UNII', 'POTENCY_AMOUNT', 'POTENCY_UNIT', 
